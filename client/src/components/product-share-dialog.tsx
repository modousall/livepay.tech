...
export function ProductShareDialog({
  ...
  const shareInfo = useMemo(() => {
    ...
    const publicCode = product.shareCode || product.id;
    const shareCode = product.shareCode || product.id.slice(0, 8).toUpperCase();
    const shareUrl = `${baseUrl}/p/${publicCode}`;
    const shopUrl = `${baseUrl}/shop/${product.vendorId}`;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-xs text-muted-foreground">Lien boutique</Label>
            <div className="flex gap-2">
              <Input
                value={shareInfo.shopUrl}
                readOnly
                className="font-mono text-xs bg-muted"
              />
              <Button
                variant="outline"
                size="icon"
                onClick={() => copyToClipboard(shareInfo.shopUrl, "Boutique")}
              >
                {copied === "Boutique" ? (
                  <Check className="w-4 h-4 text-green-600" />
                ) : (
